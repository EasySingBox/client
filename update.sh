...
if [ -z "$1" ]; then
    ...
    echo "使用方式: bash <(curl -Ls https://git.banmiya.org/easy-sing-box/client/raw/main/update.sh) <CENTRAL_API> [RANDOM_PORT_MIN] [RANDOM_PORT_MAX]"
    exit 1
fi
...
function generate_singbox_server() {
    ...
    wget --inet4-only -O "$SING_BOX_CONFIG_DIR/cert.pem" https://git.banmiya.org/easy-sing-box/client/raw/main/cert/cert.pem
    wget --inet4-only -O "$SING_BOX_CONFIG_DIR/private.key" https://git.banmiya.org/easy-sing-box/client/raw/main/cert/private.key
